...
class GUI:
    ...
    def read_data(self):
        ...
        try:
            ...
            print(field)
            pressure = float(str(ser.readline().strip()).strip("b'").split()[1]) / 1000
            print(pressure)
            for _ in range(4):
...
def write_csv():
    for i in range(1, 6):
        with open('logs\\' + now + '_Field' + str(i) + '.csv', 'a', encoding='utf-8') as csvfile:
            ...
            writer.writerow([fields['Field ' + str(i)].dates[j] for j in dates_list])
    # print('write')
...
t1 = Label([wet_air.get_center()[0] - 30, wet_air.image.get_height() * 0.82 - 15, 60, 40], 0.0, 'blue', -1, additional_symbols='%', name='wet_air')
t2 = Label([temp_air.get_center()[0] - 25, temp_air.image.get_height() * 0.84 - 15, 50, 40], 0.0, 'lightblue', -1, name='temp_air')
t3 = Label([wet_soil.get_center()[0] - 30, wet_soil.image.get_height() * 0.82 - 15, 60, 40], 0.0, 'green', -1, additional_symbols='%', name='wet_soil')
t4 = Label([temp_soil.get_center()[0] - 25, temp_soil.image.get_height() * 0.84 - 15, 50, 40], 0.0, 'lightgreen', -1, name='temp_soil')
